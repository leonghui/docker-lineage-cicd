...
if [ "$USE_MIRROR" = true ]; then
  ...
  if [ ! -d .repo ]; then
    ...
    yes | repo init -u https://github.com/LineageOS/mirror --mirror --no-clone-bundle -p linux | tee -a "$repo_log" 2>&1
  fi
  ...
  repo sync --force-sync --no-clone-bundle | tee -a "$repo_log" 2>&1
fi
...
for branch in $BRANCH_NAME; do
  ...
  if [ ! -z "$branch" ] && [ ! -z "${!device_list_cur_branch}" ]; then
    ...
    if [ "$USE_MIRROR" = true ]; then
      yes | repo init -u https://github.com/LineageOS/android.git --reference "$MIRROR_DIR" -b "$branch" | tee -a "$repo_log" 2>&1
    else
      yes | repo init -u https://github.com/LineageOS/android.git -b "$branch" | tee -a "$repo_log" 2>&1
    fi
    ...
    repo sync -c --force-sync | tee -a "$repo_log" 2>&1
